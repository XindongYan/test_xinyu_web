...
export function async(children, activityId) {
  ...
  const qaSubjectStartTime = children.find(item => item.name === 'qaSubjectStartTime');
  const qaSubjectEndTime = children.find(item => item.name === 'qaSubjectEndTime');
  ...
  if (itemSpuOption) {
    if (itemSpuOption.props.value === 'spu') {
      ...
      const bodyIndex = tempChildren.findIndex(item => item.name === 'bodySpu');
      if (bodyIndex >= 0) {
        tempChildren[bodyIndex] = {
          "className": "creator-single-item-center creator-no-label",
  ...
  if (qaSubjectStartTime && !qaSubjectStartTime.props.value) {
    const date = new Date();
    date.setHours(date.getHours() + 1);
    const index = tempChildren.findIndex(item => item.name === qaSubjectStartTime.name);
    tempChildren[index].props.value = moment(date).format('YYYY/MM/DD HH:mm:ss');
  }
  if (qaSubjectEndTime && !qaSubjectEndTime.props.value) {
    const date = new Date();
    date.setDate(date.getDate() + 1);
    date.setHours(date.getHours() + 1);
    const index = tempChildren.findIndex(item => item.name === qaSubjectEndTime.name);
    tempChildren[index].props.value = moment(date).format('YYYY/MM/DD HH:mm:ss');
  }
  return tempChildren;
}
