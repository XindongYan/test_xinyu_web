...
import { Editor, EditorState, ContentState, SelectionState, RichUtils, convertToRaw, convertFromRaw, AtomicBlockUtils, Entity, Modifier, DefaultDraftBlockRenderMap, convertFromHTML, KeyBindingUtil, getDefaultKeyBinding } from 'draft-js';
import { Icon, Divider, message, Affix, Dropdown, Menu } from 'antd';
...
export default class Editors extends PureComponent {
  ...
  componentDidMount() {
    ...
    const needDeletes = ['header-two', 'header-three', 'header-four', 'header-five', 'header-six', 'unordered-list-item', 'code-block',
    'ordered-list-item', 'blockquote', 'paragraph'];
    let customBlockRenderMap = DefaultDraftBlockRenderMap.delete('header-one');
    needDeletes.forEach(item => {
      customBlockRenderMap = customBlockRenderMap.delete(item);
    });
    this.setState({
  ...
  handleAddProduct = (auction) => {
    ...
    const contentState = editorState.getCurrentContent();
      ...
      'IMMUTABLE',
      {
        ...
        price: auction.item.finalPrice || auction.item.reservedPrice,
  ...
  handleInsertText = (text) => {
    let { editorState, contentState, selectionState } = this.state;
    const newContentState = Modifier.insertText(contentState, selectionState, text, );
    const newEditorState = EditorState.push(editorState, newContentState, 'insert-characters');

    this.setState({
      editorState: newEditorState,
    }, () => this.toggleBlockType('alignCenter'));
  }
  renderInsertNumber = () => {
    const menuList = ['— ❶ —', '— ❷ —', '— ❸ —', '— ❹ —', '— ❺ —', '— ❻ —', '— ❼ —', '— ❽ —', '— ❾ —', '— ❿ —'];
    const menu = (
      <Menu onClick={(e) => this.handleInsertText(e.key)}>
        { menuList.map(item => <Menu.Item key={item} style={{ textAlign: 'center' }}>
            <span>{item}</span>
          </Menu.Item>)
        }
      </Menu>
    ...
    return (
      <Dropdown overlay={menu} placement="bottomLeft" onVisibleChange={() => this.handleChange(this.state.editorState)}>
        <span key="cart" >
          <Icon type="tags-o" />
          插入编号
        </span>
  ...
  render() {
    ...
    const editorProps = {
      ...
      blockRenderMap: this.state.customBlockRenderMap,
      // keyBindingFn: this.myKeyBindingFn, //按键
    }
    const editorToolsWrap = <div ref="editorToolsWrap" onMouseDown={this.preventDefault} className={styles.editorToolsWrap}>
            <div>
              {props.plugins.map((item, index) => this.renderStyleTools(item, index))}
              {this.renderInsertNumber()}
            </div>
            <div className={styles.toolsLine}>
              {props.plugins.map((item, index) => this.renderTools(item, index))}
            </div>
          </div>;
    return (
      <div style={{padding: '10px 20px', marginBottom: 60}}>
        ...
        <div ref="editorBox" className={styles.editorBox} onContextMenu={this.handleContextMenu} onClick={this.handleFocus} style={{ minHeight: 320, borderBottom: '1px solid #ccc', padding: 10 }}>
